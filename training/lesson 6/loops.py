...
# Всё дело в порядке.
# Функция enumerate() всегда возвращает пару значений в строго определённом порядке:
# 1️⃣ Сначала идёт номер элемента (index)
# 2️⃣ Потом идёт само значение из списка (value)
#
# Когда ты пишешь конструкцию с запятой for a, b in ...,
# Python просто берёт то, что пришло первым, и кладёт это в первую переменную слева от запятой (a), а второе — во вторую переменную (b).
